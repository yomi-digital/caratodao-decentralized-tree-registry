...
task("return-tree", "Return tree details.")
  .setAction(async (taskArgs) => {
    ...
    if (fs.existsSync("./deployments/wallaby/CaratoDaoRegistry.json")) {
      ...
      const tokenId = "1"
      const uri = await CaratoDaoRegistryContract.tokenURI(tokenId)
      console.log("Onchain data:", uri)
      const decodedStr = JSON.parse(Buffer.from(uri.split('base64,')[1], 'base64').toString());
      console.log("Tree details:", decodedStr)
    } else {
